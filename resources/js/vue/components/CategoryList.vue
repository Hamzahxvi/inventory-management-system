    ...
    <div>
        ...
        <div v-else class="rounded-lg border">
            <table class="w-full text-sm">
                ...
                <tbody>
                    <tr v-for="cat in store.categories" :key="cat.id" class="border-b hover:bg-muted/50">
                        <td class="px-4 py-2 font-medium">{{ cat.name }}</td>
                        <td class="px-4 py-2 text-muted-foreground">{{ cat.description ?? '—' }}</td>
                        <td class="px-4 py-2 text-right">{{ cat.products_count }}</td>
                        <td class="px-4 py-2 text-right">
                            <button @click="editCategory(cat)" class="rounded p-1 hover:bg-muted text-muted-foreground hover:text-foreground">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                            </button>
                            <button @click="confirmDelete(cat)" class="rounded p-1 hover:bg-muted text-muted-foreground hover:text-destructive">
        ...
        <div v-if="showForm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="showForm = false">
            <div class="w-full max-w-md rounded-lg border bg-card p-6 shadow-lg">
                <h3 class="mb-4 text-lg font-semibold">{{ editing ? 'Edit Category' : 'Add Category' }}</h3>
                <form @submit.prevent="saveCategory" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea v-model="form.description" rows="2" class="w-full rounded-md border px-3 py-2 text-sm"></textarea>
                    </div>
