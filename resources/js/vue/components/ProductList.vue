<template>
    <div>
        ...
        <div class="mb-4 flex flex-wrap items-center gap-3">
            ...
            <button
                @click="exportCsv"
                class="ml-auto rounded-md border px-3 py-2 text-sm hover:bg-muted"
            >
                Export CSV
            </button>
        </div>
                ...
                <p class="text-sm text-muted-foreground">
                    ...
                    {{
                        store.products.length > 0
                            ? (store.currentPage - 1) * store.filters.per_page +
                              1
                            : 0
                    }}–{{
                        Math.min(
                            store.currentPage * store.filters.per_page,
                            store.total,
                        )
                    }}
                    of {{ store.total }}
...
function toggleSort(field: string) {
    if (store.filters.sort === field) {
        store.filters.direction =
            store.filters.direction === 'asc' ? 'desc' : 'asc';
    } else {
